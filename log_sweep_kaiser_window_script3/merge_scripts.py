...
script3 = np.genfromtxt('log_sweep_kaiser_window_script3.txt')

script3_1 = np.genfromtxt('log_sweep_kaiser_window_script3_1.txt')
...
NFFT_bandstop = 88201
max_measurement = 362.799629307

plt.plot(
    fade_in_list,
    -10 * np.log10(1 / NFFT_bandstop * np.abs(
    np.asarray(spectrum_distance_list) / max_measurement)),
    label='Deconvolution: Excitation windowed')

plt.plot(fade_in_list,
         -10 * np.log10(1 / NFFT_bandstop * np.abs(np.asarray(spectrum_distance_unwindowed_deconvolution_list) /
