...
script5 = np.genfromtxt('log_sweep_kaiser_window_bandlimited_script5.txt')

script5_1 = np.genfromtxt('log_sweep_kaiser_window_bandlimited_script5_1.txt')
...
NFFT_dirac = 88201
max_measurement = 7040966.76862

plt.plot(
    fade_in_list,
     -10 * np.log10(1 / NFFT_dirac * np.abs(
         np.asarray(spectrum_distance_list) / max_measurement)),
     label='Deconvolution: Excitation windowed')

plt.plot(fade_in_list,
         -10 * np.log10(1 / NFFT_dirac * np.abs(np.asarray(spectrum_distance_unwindowed_deconvolution_list) /
...
plt.xlim([-10, 1000])
plt.savefig('spectrale_distance.png')
